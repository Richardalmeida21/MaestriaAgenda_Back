-- =====================================================
-- FUNÇÕES POSTGRESQL - VERSÃO CORRIGIDA FINAL
-- =====================================================
-- Corrigido: valor calculado via JOIN (agendamento → agendamento_servico → servico)
...
BEGIN
    ...
    valores_calculados AS (
        SELECT
            ...
            SUM(valor_servico * (comissao_percentual / 100)) as total_comissao,
            SUM(
                CASE 
                    WHEN forma_pagamento IN ('CREDITO', 'DEBITO', 'PIX') THEN
                        valor_servico * COALESCE(
                            (SELECT taxa_percentual FROM taxa_pagamento 
                             WHERE forma_pagamento = av.forma_pagamento 
...
BEGIN
    ...
    JOIN agendamento_servico ags ON a.id = ags.agendamento_id
    JOIN servico s ON ags.servico_id = s.id
    WHERE a.data BETWEEN p_data_inicio AND p_data_fim
      AND a.pago = true;
...
-- ✅ FUNÇÕES CORRIGIDAS!
-- =====================================================
-- Agora o cálculo de valor está correto usando JOINs:
-- agendamento → agendamento_servico → servico.valor
